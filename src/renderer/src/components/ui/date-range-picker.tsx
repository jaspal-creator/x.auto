...
export const DateRangePicker: FC<DateRangePickerProps> & {
  ...
  const { t, i18n } = useTranslation(['utils'], { keyPrefix: 'date_range_picker' });

  const [_locale, setLocale] = React.useState<Locale>();

  React.useEffect(() => {
    i18n.language === 'ro' && setLocale(ro);
    i18n.language === 'ru' && setLocale(ru);
    i18n.language === 'en' && setLocale(enUS);
  }, [i18n.language]);
  ...
  return (
    ...
    >
      <PopoverTrigger asChild>
        <Button
          variant="outline"
          className="w-full m-0 h-fit p-3 rounded-2xl border-2 border-[#E1E1E1] text-xl font-normal shadow-md"
        >
          <div className="">
            <div className="">
              <div>{`${formatDate(range.from, _locale?.code)}${
                range.to != null ? ' - ' + formatDate(range.to, _locale?.code) : ''
              }`}</div>
            </div>
            {rangeCompare != null && (
              <div className="">
                <>
                  vs. {formatDate(rangeCompare.from, _locale?.code)}
                  {rangeCompare.to != null
                    ? ` - ${formatDate(rangeCompare.to, _locale?.code)}`
                    : ''}
                </>
              </div>
            )}
      ...
      <PopoverContent align={align} className="w-fit  m-0 p-6">
        <div className="flex">
          <div className="flex">
            <div className="flex flex-col">
              <div className="flex flex-col lg:flex-row gap-1  justify-end items-center lg:items-start">
                {showCompare && (
          ...
          {!isSmallScreen && (
            <div className="flex flex-col items-end gap-1 border-l-2 border-foreground/10 ml-6 pl-6">
