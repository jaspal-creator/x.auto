...
import { useGetCarReports } from '@/features/cars/hooks/useGetCarReports';
...
export default function Car(): React.ReactNode {
  const { t } = useTranslation(['cars'], { keyPrefix: 'table.car' });
  const { t: tInvoices } = useTranslation(['invoices']);
  const { id } = useParams();
  const { query, setSearch, setSort, setPage, getTotalPages, page } = useFiltration();
  const [type, setType] = React.useState<Readonly<'MASTER' | 'CUSTOMER'>>('CUSTOMER');
  const { reports, loading } = useGetCarReports({ id: id as string, query: { ...query, type } });
  const { deleteReport } = useDeleteReport({
    invalidateKeys: ['car', 'reports', id, { ...query, type }]
  });
  ...
  return (
    <CarsLayout>
      ...
      <Utilities<typeof type>
        title={`${reports?.data?.car?.brand} ${reports?.data?.car?.model} (${reports?.data?.car?.year})`}
        search={setSearch}
        sort={setSort}
        categories={['CUSTOMER', 'MASTER']}
